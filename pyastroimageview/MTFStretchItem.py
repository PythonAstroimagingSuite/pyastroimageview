...
#
# Based on GradientEditorItem from pgqtgraph!
#
import logging
import weakref
import numpy as np
...
class MTFSliderItem(GraphicsWidget):
    ...
    def addTick(self, x, color=None, movable=True, removable=True):
        ## public
        """
        ...
        **Arguments:**
        ...
        tick = MTFTick(self, [x*self.length, 0], color, movable, removable,
                       self.tickSize, pen=self.tickPen)
        self.ticks[tick] = x
...
class ignorefornow:
    class GradientEditorItem(MTFSliderItem):
        """
        ...
        **Signals:**
        ...
        def __init__(self, *args, **kargs):
            ...
            **Arguments:**
            ...
            MTFSliderItem.__init__(self, *args, **kargs)
        ...
        def setOrientation(self, orientation):
            ...
            **Arguments:**
            ...
            ==============  ===================================================================
            """
            MTFSliderItem.setOrientation(self, orientation)
        ...
        def setLength(self, newLen):
            ...
            MTFSliderItem.setLength(self, newLen)
        ...
        def tickMoved(self, tick, pos):
            #private
            MTFSliderItem.tickMoved(self, tick, pos)
            self.updateGradient()
        ...
        def mouseReleaseEvent(self, ev):
            #private
            MTFSliderItem.mouseReleaseEvent(self, ev)
            self.updateGradient()

        def addTick(self, x, color=None, movable=True, removable=True, finish=True):
            """
            ...
            **Arguments:**
            ...
            t = MTFSliderItem.addTick(self, x, color=color, movable=movable)
            t.colorChangeAllowed = True
            t.removeAllowed = removable
        ...
        def removeTick(self, tick, finish=True):
            MTFSliderItem.removeTick(self, tick)
            if finish:
                self.updateGradient()
                self.sigGradientChangeFinished.emit(self)
...
class MTFTick(QtGui.QGraphicsWidget):  ## NOTE: Making this a subclass of GraphicsObject instead results in
